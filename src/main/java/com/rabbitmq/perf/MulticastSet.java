...
public class MulticastSet {
  ...
  public static final int DEFAULT_CONSUMER_WORK_SERVICE_THREAD_POOL_SIZE =
  ...
  public MulticastSet(
    ...
    this.params = params;
    this.testID = testID;
    this.uris = uris == null || uris.isEmpty() ? null : new CopyOnWriteArrayList<>(uris);
    ...
    if (this.params.getPublishingRates() == null || this.params.getPublishingRates().isEmpty()) {
      ...
      ScheduledExecutorService scheduledExecutorService =
          this.threadingHandler.scheduledExecutorService("perf-test-variable-rate-scheduler", 1);
      this.rateIndicator =
    ...
    if (this.params.getConsumerLatencies() == null
        || this.params.getConsumerLatencies().isEmpty()) {
      this.consumerLatencyIndicator =
          new FixedValueIndicator<>(params.getConsumerLatencyInMicroseconds());
    } else {
      ScheduledExecutorService scheduledExecutorService =
          this.threadingHandler.scheduledExecutorService(
  ...
  protected static int nbThreadsForConsumer(MulticastParams params) {
    ...
    // one thread for each channel when channel number is <=
  ...
  protected static int nbThreadsForProducerScheduledExecutorService(MulticastParams params) {
    ...
    if (producerExecutorServiceNbThreads <= 0) {
      ...
      /**
       * Why 100? This is arbitrary. We assume 1 thread is more than enough to handle the publishing
       * of 100 messages in 1 second, the fastest rate being 10 messages / second when using
       * --publishing-interval. Then, a new thread is used every for every 100 messages / second.
       * This is 21 threads for 1000 producers publishing 1 message / second, which seems
       * reasonable. There's a command line argument to override this anyway.
       */
      int threadCount = (int) (rate / 100d) + 1;
      LOGGER.debug(
          "Using {} thread(s) to schedule {} publisher(s) publishing every {} ms",
          threadCount,
          producerThreadCount,
  ...
  public void run(boolean announceStartup)
    ...
    if (waitUntilBrokerAvailableIfNecessary(
      ...
      ExecutorService executorServiceConfigurationConnection =
          this.threadingHandler.executorService("perf-test-configuration-", 1);
      ...
      List<MulticastParams.TopologyHandlerResult> topologyHandlerResults =
          params.configureAllQueues(configurationConnections);
      ...
      if (!configurationConnections.isEmpty()
          && Utils.isRecoverable(configurationConnections.get(0))) {
        topologyRecordingScheduledExecutorService =
            this.threadingHandler.scheduledExecutorService("perf-test-topology-recovery-", 1);
      }
      ...
      ExecutorService executorServiceForProducersConsumers =
          this.threadingHandler.executorService("perf-test-producers-worker-", 0);
        ...
        shutdownSequence =
            this.shutdownService.wrap(
                () -> {
                  ...
                  if (!done) {
                    LOGGER.debug(
                        "Shutdown not completed in {} second(s), aborting.", shutdownTimeout);
            ...
            () -> {
              for (Consumer consumer : consumerRunnables) {
                try {
                  ...
                  LOGGER.info(
                      "Error while checking exit-when for consumer {}: {}",
                      consumer,
                      e.getMessage());
                }
              }
  ...
  static boolean waitUntilBrokerAvailableIfNecessary(
      ...
      while ((System.nanoTime() - started) / 1_000_000_000 < startUpTimeoutInSeconds) {
        ...
        while (iterator.hasNext()) {
          ...
          try (Connection ignored = factory.newConnection("perf-test-test")) {
            ...
            LOGGER.info(
                "Could not connect to broker " + factory.getHost() + ":" + factory.getPort());
          }
        }
        Thread.sleep(1000L);
      }
  ...
  private Function<Integer, ExecutorService> createConsumersExecutorsFactory() {
    Function<Integer, ExecutorService> consumersExecutorsFactory;
    if (params.isPolling()) {
      // polling, i.e. using basic.get in a loop, we need a dedicated thread for each channel for
      // each consumer
      // FIXME we keep also an extra thread for the connection factory consumer dispatcher, which
      // sometimes
      // does not close properly without some room and makes the channel manager complain
      consumersExecutorsFactory =
          consumerNumber ->
              this.threadingHandler.executorService(
  ...
  private void createProducers(
    ...
    ExecutorService executorService = Executors.newFixedThreadPool(producerConnections.length);
    CountDownLatch allCreated = new CountDownLatch(producerConnections.length);
    List<AgentState> states = new CopyOnWriteArrayList<>();
    IntStream.range(0, producerConnections.length * params.getProducerChannelCount())
        .forEach(ignored -> states.add(null));
          ...
          (Callable<Void>)
              () -> {
                Connection producerConnection =
                    createConnection(PRODUCER_THREAD_PREFIX + producerIndex);
                producerConnections[producerIndex] = producerConnection;
                for (int j = 0; j < params.getProducerChannelCount(); j++) {
                  ...
                  agentState.runnable =
                      params.createProducer(
                          ...
                          this.rateIndicator,
                          this.messageSizeIndicator);
                  states.set((producerIndex * params.getProducerChannelCount()) + j, agentState);
                }
  ...
  private void startConsumers(Runnable[] consumerRunnables) throws InterruptedException {
    if (this.params.getConsumerStartDelay().getSeconds() <= 0) {
      ...
      this.threadingHandler
          .scheduledExecutorService("consumer-start-delay-", 0)
          .schedule(
              () -> {
                this.consumerLatencyIndicator.start();
                for (Runnable runnable : consumerRunnables) {
  ...
  private void startProducers(AgentState[] producerStates) {
    ...
    if (!this.rateIndicator.isVariable() && rateIndication >= 1.0f && rateIndication <= 10.0f) {
      ...
      LOGGER.debug(
          "Rate between 1 and 10 messages / second, "
              + "falling back to scheduling with {} ms as publishing interval",
          calculatedPublishingInterval.toMillis());
    ...
    if (params.getPublishingInterval() != null) {
      ScheduledExecutorService producersExecutorService =
          this.threadingHandler.scheduledExecutorService(
      ...
      if (params.getProducerRandomStartDelayInSeconds() > 0) {
        LOGGER.debug(
            "Using random start-up delay for producers, from 1 ms to {} s",
      ...
      for (int i = 0; i < producerStates.length; i++) {
        ...
        producerState.task =
            producersExecutorService.scheduleAtFixedRate(
      ...
      if (!this.rateIndicator.isVariable() && this.rateIndicator.getValue() == 0.0f) {
        ...
        ExecutorService producersExecutorService =
            this.threadingHandler.executorService(PRODUCER_THREAD_PREFIX, producerStates.length);
        for (AgentState producerState : producerStates) {
  ...
  private void shutdown(
      List<Connection> configurationConnections,
      Connection[] consumerConnections,
      AgentState[] producerStates,
      Connection[] producerConnections) {
    try {
      ...
      for (AgentState producerState : producerStates) {
        if (producerState.task != null && !producerState.task.isDone()) {
          try {
            ...
            LOGGER.debug(
                "Error while waiting for producer to stop: {}. Moving on.", e.getMessage());
          }
        }
      }
  ...
  private void enableTopologyRecoveryIfNecessary(
      List<MulticastParams.TopologyHandlerResult> topologyHandlerResults) throws IOException {
  ...
  /** Abstraction for thread management. Exists to ease testing. */
  ...
  static class DefaultThreadingHandler implements ThreadingHandler {
    ...
    public ExecutorService executorService(String name, int nbThreads) {
      if (nbThreads <= 0) {
        return create(
            () -> Executors.newSingleThreadExecutor(new NamedThreadFactory(prefix + name)));
      } else {
        return create(
            () -> Executors.newFixedThreadPool(nbThreads, new NamedThreadFactory(prefix + name)));
      }
    }
    ...
    public ScheduledExecutorService scheduledExecutorService(String name, int nbThreads) {
      return (ScheduledExecutorService)
          create(() -> Executors.newScheduledThreadPool(nbThreads, new NamedThreadFactory(name)));
    }
      ...
      if (closing.compareAndSet(false, true)) {
        for (ExecutorService executorService : executorServices) {
          ...
          try {
            ...
            if (!terminated) {
              LoggerFactory.getLogger(DefaultThreadingHandler.class)
                  .warn("Some PerfTest tasks (producer, consumer, rate scheduler) didn't finish");
            }
          } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new RuntimeException(e);
  ...
  static class CacheConsumersExecutorsFactory implements Function<Integer, ExecutorService> {
    ...
    CacheConsumersExecutorsFactory(
        ThreadingHandler threadingHandler, MulticastParams params, int modulo) {
      this.threadingHandler = threadingHandler;
      this.params = params;
      this.modulo = modulo;
