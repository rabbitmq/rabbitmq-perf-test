...
public class VaryingScenarioStats implements ScenarioStats {
  private final Map<List<VariableValue>, SimpleScenarioStats> stats =
      new HashMap<List<VariableValue>, SimpleScenarioStats>();
    ...
    for (List<VariableValue> key : keys) {
      ...
      for (int i = 0; i < key.size(); i++) {
        ...
        if (i == key.size() - 1) {
          ...
          node =
              (Map<String, Object>)
                  get(elem.getValue().toString(), node, new HashMap<String, Object>());
        }
      }
    }
    map.put("data", data);
